...
public enum Key {
    ...
    // Numbers (top row)
    NUM_0(GLFW_KEY_0), NUM_1(GLFW_KEY_1), NUM_2(GLFW_KEY_2),
    NUM_3(GLFW_KEY_3), NUM_4(GLFW_KEY_4), NUM_5(GLFW_KEY_5),
    NUM_6(GLFW_KEY_6), NUM_7(GLFW_KEY_7), NUM_8(GLFW_KEY_8),
    NUM_9(GLFW_KEY_9),
    ...
    private static final IntIntHashMap LOOKUP = new IntIntHashMap();

    static {
        for (Key key : values()) {
            LOOKUP.put(key.glfwKey, key.ordinal());
        }
    }
    ...
    public static Key fromGlfw(int glfwKey) {
        int ord = LOOKUP.getOrDefault(glfwKey, -1);
        if (ord == -1) return UNKNOWN;
